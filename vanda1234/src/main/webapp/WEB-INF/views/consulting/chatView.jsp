	...
	<div class="inbox_people">
	  ...
	  <div class="inbox_chat scroll">
		  ...
		  <c:choose>
		  	...
		  	<c:when test="${user.user_rule eq '회원' }">
		  	...
		  	<c:when test="${user.user_rule eq '수의사' }">
...
<script type="text/javascript">
	$(document).ready(function(){
		var userId = "${user.user_id}"; // 유저 아이디
		var roomNum; // 방 번호
		...
		$(".msg_send_btn").on("click",function(e){
			if(!msgInput.val()){
				...
				chatService.add({room_num:roomNum,user_id:"${user.user_id}",msg_content:msgInput.val()},function(result){
					sendMessage();
		...
		/* 소켓 통신  */
		let sock = new SockJS("http://localhost:8080/chat/chatView");
		sock.onmessage = onMessage;
		sock.onclose = onClose;

		// 메시지 전송
		function sendMessage() {				
			sock.send(msgInput.val()+",${user.user_id}");
		}
		// 서버로부터 메시지를 받았을 때
